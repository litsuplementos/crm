...
const ClientesView = (() => {
  ...
  // Cargar clientes + historial acumulado
  async function load() {
    const [
      { data: clientes, error: errC },
      { data: historial, error: errH }
    ] = await Promise.all([
      db.from('clientes')
  ...
  function render() {
    ...
    document.getElementById('clientes-table-count').textContent =
      `${total} clientes`;
    ...
    for (const c of page) {
      ...
      // Al hacer clic → abrir modal de historial del cliente
  ...
  async function openClienteHistorial(c) {
    // Cargar historial mensual + ventas del cliente
    const [
      { data: hist },
      { data: ventasCliente }
    ] = await Promise.all([
    ...
    const ventasRows = (ventasCliente || []).map(v => {
      ...
      return `
        <tr onclick="closeClienteHistorialModal();setTimeout(()=>showNuevoRegistro(${v.id}),50)"
          ...
          <td style="padding:8px 12px;font-size:11px;color:var(--accent2);">
            ${v.agente?.nombre || '—'}
          </td>
        </tr>`;
    }).join('') || `<tr><td colspan="6" style="padding:20px;text-align:center;color:var(--text3);">Sin registros</td></tr>`;

    // Insertar en modal reutilizado (stat-modal)
    ...
    document.getElementById('stat-modal-body').innerHTML = `
      ...
      <div style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:18px;">
        <div class="stat-card" style="flex:1;min-width:120px;padding:12px;">
          <div style="font-size:11px;color:var(--text3);text-transform:uppercase;
              font-weight:700;letter-spacing:0.5px;margin-bottom:4px;">Unidades totales</div>
          <div style="font-size:24px;font-weight:800;color:var(--blue);
              font-family:'Syne',sans-serif;">${totalUnid}</div>
        </div>
        <div class="stat-card" style="flex:1;min-width:120px;padding:12px;">
          <div style="font-size:11px;color:var(--text3);text-transform:uppercase;
              font-weight:700;letter-spacing:0.5px;margin-bottom:4px;">Monto total</div>
          <div style="font-size:24px;font-weight:800;color:var(--green);
              font-family:'Syne',sans-serif;">Bs.${totalMonto.toFixed(0)}</div>
        </div>
        <div class="stat-card" style="flex:1;min-width:120px;padding:12px;">
          <div style="font-size:11px;color:var(--text3);text-transform:uppercase;
              font-weight:700;letter-spacing:0.5px;margin-bottom:4px;">Faltas / Sin resp.</div>
          <div style="font-size:24px;font-weight:800;color:${c.faltas > 0 ? 'var(--red)' : 'var(--text3)'};
              font-family:'Syne',sans-serif;">${c.faltas} / ${c.sin_respuesta}</div>
        </div>
        <div class="stat-card" style="flex:1;min-width:120px;padding:12px;">
          <div style="font-size:11px;color:var(--text3);text-transform:uppercase;
              font-weight:700;letter-spacing:0.5px;margin-bottom:4px;">Ubicación</div>
      ...
      <div style="font-size:12px;font-weight:700;color:var(--text3);text-transform:uppercase;
          letter-spacing:0.5px;margin-bottom:8px;">📅 Historial mensual</div>
      <div style="overflow-x:auto;margin-bottom:20px;">
        <table style="width:100%;border-collapse:collapse;">
          <thead>
            <tr>
              <th style="background:var(--surface2);padding:7px 12px;text-align:left;
                  font-size:11px;font-weight:700;color:var(--text3);text-transform:uppercase;
                  letter-spacing:0.5px;border-bottom:1px solid var(--border);">Mes</th>
              <th style="background:var(--surface2);padding:7px 12px;text-align:left;
                  font-size:11px;font-weight:700;color:var(--text3);text-transform:uppercase;
                  letter-spacing:0.5px;border-bottom:1px solid var(--border);">Unidades</th>
              <th style="background:var(--surface2);padding:7px 12px;text-align:left;
                  font-size:11px;font-weight:700;color:var(--text3);text-transform:uppercase;
                  letter-spacing:0.5px;border-bottom:1px solid var(--border);">Monto</th>
              <th style="background:var(--surface2);padding:7px 12px;text-align:left;
                  font-size:11px;font-weight:700;color:var(--text3);text-transform:uppercase;
                  letter-spacing:0.5px;border-bottom:1px solid var(--border);">Transacciones</th>
            </tr>
          </thead>
      ...
      <!-- Registros recientes -->
      <div style="font-size:12px;font-weight:700;color:var(--text3);text-transform:uppercase;
          letter-spacing:0.5px;margin-bottom:8px;">📋 Registros recientes (máx. 30)</div>
      <div style="overflow-x:auto;">
        <table style="width:100%;border-collapse:collapse;">
          <thead>
            <tr>
              <th style="background:var(--surface2);padding:7px 12px;text-align:left;font-size:11px;
                  font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:0.5px;
                  border-bottom:1px solid var(--border);">Fecha creación</th>
              <th style="background:var(--surface2);padding:7px 12px;text-align:left;font-size:11px;
                  font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:0.5px;
                  border-bottom:1px solid var(--border);">Estado</th>
              <th style="background:var(--surface2);padding:7px 12px;text-align:left;font-size:11px;
                  font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:0.5px;
                  border-bottom:1px solid var(--border);">Productos</th>
              <th style="background:var(--surface2);padding:7px 12px;text-align:left;font-size:11px;
                  font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:0.5px;
                  border-bottom:1px solid var(--border);">Monto</th>
              <th style="background:var(--surface2);padding:7px 12px;text-align:left;font-size:11px;
                  font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:0.5px;
                  border-bottom:1px solid var(--border);">Última actualización</th>
              <th style="background:var(--surface2);padding:7px 12px;text-align:left;font-size:11px;
                  font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:0.5px;
                  border-bottom:1px solid var(--border);">Agente</th>
            </tr>
          </thead>
  ...
  return { load, render, goPage, debouncedRender, openClienteHistorial };
})();
